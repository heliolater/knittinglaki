...
abstract class _$AppDatabase extends GeneratedDatabase {
  ...
  @override
  List<DatabaseSchemaEntity> get allSchemaEntities => [projects, counters];
  @override
  StreamQueryUpdateRules get streamUpdateRules => const StreamQueryUpdateRules([
    WritePropagation(
      ...
      result: [TableUpdate('counters', kind: UpdateKind.delete)],
    ),
  ]);
}
...
final class $$ProjectsTableReferences
  ...
  static MultiTypedResultKey<$CountersTable, List<Counter>> _countersRefsTable(
    _$AppDatabase db,
  ) => MultiTypedResultKey.fromTable(
    db.counters,
    aliasName: 'projects__id__counters__project_id',
  );

  $$CountersTableProcessedTableManager get countersRefs {
    final manager = $$CountersTableTableManager(
      $_db,
      $_db.counters,
    ).filter((f) => f.projectId.id.sqlEquals($_itemColumn<int>('id')!));

    final cache = $_typedResult.readTableOrNull(_countersRefsTable($_db));
...
class $$ProjectsTableFilterComposer
  ...
  Expression<bool> countersRefs(
    Expression<bool> Function($$CountersTableFilterComposer f) f,
  ) {
...
class $$ProjectsTableAnnotationComposer
  ...
  Expression<T> countersRefs<T extends Object>(
    Expression<T> Function($$CountersTableAnnotationComposer a) f,
  ) {
...
class $$ProjectsTableTableManager
    extends
        RootTableManager<
          ...
          Project,
          PrefetchHooks Function({bool countersRefs})
        > {
    ...
    : super(
        TableManagerState(
          ...
          withReferenceMapper: (p0) => p0
              ...
              .toList(),
          prefetchHooksCallback: ({countersRefs = false}) {
            return PrefetchHooks(
              db: db,
              explicitlyWatchedTables: [if (countersRefs) db.counters],
              addJoins: null,
              getPrefetchedDataCallback: (items) async {
                return [
                  if (countersRefs)
                    await $_getPrefetchedData<Project, $ProjectsTable, Counter>(
                      currentTable: table,
                      referencedTable: $$ProjectsTableReferences
                          ._countersRefsTable(db),
                      managerFromTypedResult: (p0) =>
                          $$ProjectsTableReferences(db, table, p0).countersRefs,
...
final class $$CountersTableReferences
    extends BaseReferences<_$AppDatabase, $CountersTable, Counter> {
  $$CountersTableReferences(super.$_db, super.$_table, super.$_typedResult);

  static $ProjectsTable _projectIdTable(_$AppDatabase db) =>
      db.projects.createAlias('counters__project_id__projects__id');
...
class $$CountersTableTableManager
    extends
        RootTableManager<
          ...
          (Counter, $$CountersTableReferences),
          Counter,
    ...
    : super(
        TableManagerState(
          ...
          withReferenceMapper: (p0) => p0
              .map(
                (e) => (
                  e.readTable<$CountersTable, Counter>(table),
                    ...
                    if (projectId) {
                      state = state.withJoin(
                        ...
                        referencedTable: $$CountersTableReferences
                            ._projectIdTable(db),
                        referencedColumn: $$CountersTableReferences
                            ._projectIdTable(db)
                            .id,
                      ) as T;
                    }
